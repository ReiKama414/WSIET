...
class Carousel {
  constructor(el, _options = {}) {
    ...
    this.carousel = el;
    this.settings = { ...DEFAULTS, ..._options };
    this.slides = document.querySelectorAll(this.settings.slideSelector);
    this.state = { currentSlide: this.settings.initialSlideIndex };
    this.track = null;
  ...
  createSlideTrack() {
    const { slides, carousel, track } = this; // Do not re-create track if it exists
    if (!track) {
  ...
  createNavigationDot(index) {
    ...
    navigationDot.classList.add("ht");
  ...
  createNavigationButtons() {
    const createNavigationButton = (buttonType) => {
      ...
      navigationButton.classList.add('ht');
